...
import com.github.morotsman.lote.api.spi.{Overlay, Slide, Terminal => TerminalAlgebra}
...
class SessionBuilderSpec extends FunSuite {
  ...
  test("run() requires at least one slide") {
    implicit val dummyTerminal: TerminalAlgebra[IO] = null // never used — require fires first
    intercept[IllegalArgumentException] {
