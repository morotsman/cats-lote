  ...
  override def alignText(
    ...
    val horizontallyAligned = alignment.horizontalAlignment match {
      case HorizontalAlignment.Left =>
        lines.map(l => l + " " * (screen.screenWidth - l.length))
      ...
      case HorizontalAlignment.Right =>
        lines.map(l => " " * (screen.screenWidth - l.length) + l)
    }
    ...
    val verticallyAligned = alignment.verticalAlignment match {
      case VerticalAlignment.Up =>
        horizontallyAligned ++ Array.fill(screen.screenHeight - contentHeight)(
          " " * screen.screenWidth
        )
      case VerticalAlignment.Center =>
        val topPadding = (screen.screenHeight - contentHeight) / 2
        Array.fill(topPadding)(
          " " * screen.screenWidth
        ) ++ horizontallyAligned ++
          Array.fill(screen.screenHeight - contentHeight - topPadding)(
            " " * screen.screenWidth
          )
      case VerticalAlignment.Down =>
        Array.fill(screen.screenHeight - contentHeight)(
          " " * screen.screenWidth
        ) ++ horizontallyAligned
    }
