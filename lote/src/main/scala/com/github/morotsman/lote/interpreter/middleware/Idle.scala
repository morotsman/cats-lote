...
object Idle {
  ...
  def make[F[_] : Temporal : Ref.Make](
    ...
    private def updateBugs(context: Screen, s: IdleOverlayState, now: Long, screenContent: String): IdleOverlayState = {
      val rng = s.random
        ...
        for {
          ...
          updatedState = {
            val withIdleStart: IdleOverlayState = if (!currentState.wasIdle) {
              currentState.copy(wasIdle = true, idleStartTime = idleStart.getOrElse(now), lastSpawnTime = now)
            } else currentState
