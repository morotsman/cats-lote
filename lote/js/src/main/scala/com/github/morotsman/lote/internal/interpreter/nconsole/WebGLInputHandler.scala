...
/** Bridges DOM input events (keyboard, mouse, paste) into an effectful `Queue[F, Int]`.
  *
  * Produces the same character/escape sequences as JLine and xterm.js so the rest of
  * the framework can handle input uniformly across all backends.
  */
private[nconsole] object WebGLInputHandler {

  /** Wire up keyboard, mouse, and paste listeners on the given canvas element.
    *
    * @return a reference to the keydown listener so it can be removed on close
    */
  def attach[F[_]](
    ...
    // ---- Paste ----
    canvas.addEventListener("paste", { (ev: Event) =>
