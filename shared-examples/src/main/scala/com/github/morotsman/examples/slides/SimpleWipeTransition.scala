...
/** A minimal custom transition that replaces each character with a space, one row at a time from top to bottom, then
  * reveals the next slide.
  *
  * This version uses `TickedTransition` to eliminate the manual ticker subscribe/unsubscribe, FixedStep, and Deferred
  * boilerplate.
  *
  * ==How it works==
  *
  *   1. The helper reads both slides' content and drives a progress value from 0.0 to 1.0 over `totalSteps` simulation
  *      steps.
  *   2. Our `renderFrame` function maps progress to "how many rows from the top show the new slide vs. the old slide".
  *   3. When progress reaches 1.0, the helper automatically completes the transition and shows the final content.
  *
  * ==Usage==
  * {{{
  * // In a SessionBuilder:
...
object SimpleWipeTransition {
  ...
  /** Wraps construction in a `Contextual` so the transition receives the shared console, ticker, and animation settings
