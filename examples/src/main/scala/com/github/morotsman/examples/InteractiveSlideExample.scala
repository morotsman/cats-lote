      ...
      .addTextSlide {
        _.content(
          """A custom `Slide[F]` is added with `addSlide(...)` or `addSlideF(...)`.
            ...
            |
            |This example uses that pattern to create the animator first, because stateful behavior rarely assembles itself out of principle.""".stripMargin
        ).title("How To Use It")
      }
      .addTextSlide {
        _.content(
          """A custom `Slide[F]` usually has four moving parts:
            ...
            |- `userInput` for keys and other events while it is visible
            |
            |In other words, the slide gets a lifecycle, which is a very serious thing to give a terminal toy.""".stripMargin
      ...
      .addTextSlide {
        _.content(
          """This particular example is a complete interactive demo.
            ...
            |
              |Collect the `?` characters and try not to run into yourself, which remains good advice even outside software.""".stripMargin
      ...
      .addTextSlide {
        _.content(
          """If you want to experiment with the feel of the game:
            ...
            |
            |AdvancedExample shows the same idea inside a larger presentation, because apparently one tiny terminal game was only the beginning.""".stripMargin
        ).title("What To Tweak")
      }
      .run()
}
